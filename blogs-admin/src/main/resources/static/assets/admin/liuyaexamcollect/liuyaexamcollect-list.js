...
layui.config({base : "assets/"}).extend({"liuyaexamcollect_config":"admin/liuyaexamcollect/liuyaexamcollect-config","magicalcoderlist" : "magicalcoder/v001/rmp/magicalcoderlist"})
layui.use(["magicalcoderlist","liuyaexamcollect_config"],function(){
    var config = $.extend(layui.liuyaexamcollect_config,{
        layTable : {
            //表格内容
            elem : '#newsList',
            id : "newsListTable"
            ...
            //排序跟数据库实际字段名的映射
        sortMap:{
            id:'id',examId:'exam_id',userId:'user_id',collectCreatetime:'collect_createtime'
        }
    });
    ...
    var cols = [[
        {type: "checkbox", fixed:"left", width:50},
            ...
            {field: 'examId', title: '收藏的题目id', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ magicalcoderlist.escapeHTML(d.examId) +'" class="magicalcoder-table-text layui-input security_list_table_form_examId" name="examId" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="收藏的题目id"/>'
                }
                , sort:true
            },
